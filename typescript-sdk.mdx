...
You can install the Channel3 SDK using your favorite package manager: npm, Yarn, or pnpm.
...
<Warning>
  All methods on the `Channel3Client` (like `search` and `getProduct`) are
  asynchronous and return Promises. You'll need to use `async/await` or
  `.then()/.catch()` to work with the results.
</Warning>
...
async function searchByBase64Image() {
  try {
    ...
    if (products.length > 0) {
      console.log(
        `Found ${products.length} products similar to the local image.`
      );
...
<Note type="info">
  **Image URL vs. Base64:** - `imageUrl`: Ideal if the image is hosted online. -
  `base64Image`: Use when you have the image data directly (e.g., user upload,
...
async function searchWithFilters() {
  const filters: SearchFilters = {
    // Brand filtering (use IDs)
    brand_ids: ["brand_id_1", "brand_id_2"],

    // Price filtering (values are inclusive)
    price: {
      min_price: 50.0,
      max_price: 200.0,
    },
...
<Check>
  **Available Filters:** - **brand_ids**: Array of brand IDs - **price**: Object
  with `min_price` / `max_price` (inclusive) - **gender**: Target gender
  ("male", "female", "unisex") - **availability**: Array of stock statuses
  (["InStock", "LimitedAvailability", "PreOrder", "BackOrder", "SoldOut",
...
async function getProductDetail(productId: string) {
  try {
    ...
    if (productDetail.price) {
      console.log(
        `\nPrice: $${productDetail.price.price} ${productDetail.price.currency}`
      );
      if (productDetail.price.compare_at_price) {
        console.log(`Compare at: $${productDetail.price.compare_at_price}`);
      }
    }
...
<Note title="Product ID">
  The `productId` is a unique string identifier for each product. You'll get
  this from the `id` field of `Product` objects returned by the `search` method.
</Note>
...
async function getAllBrands() {
  try {
    // Get all brands (paginated response)
    const brandsPage = await client.getBrands();
    console.log(
      `Total brands: ${brandsPage.total} (page ${brandsPage.page}, size ${brandsPage.size})`
    );

    console.log(`Items:`);
    brandsPage.items.forEach((brand: Brand) => {
...
async function searchBrands() {
  try {
    ...
    const athleticBrandsPage = await client.getBrands("athletic");
    console.log(
      `Found ${athleticBrandsPage.total} athletic brands (showing ${athleticBrandsPage.items.length})`
    );

    // Get brands with pagination
    const page2Brands = await client.getBrands(undefined, 2, 50);
    console.log(`Page 2 brands: ${page2Brands.items.length} results`);

    // Combined filtering and pagination
    const filteredBrands = await client.getBrands("premium", 1, 25);
    console.log(
      `Premium brands (page 1, 25 per page): ${filteredBrands.items.length} results`
    );

    return filteredBrands.items;
  } catch (error) {
...
async function searchWithBrandFilters() {
  try {
    ...
    const athleticBrandsPage = await client.getBrands("athletic");
    const brandIds = athleticBrandsPage.items
      .slice(0, 3)
      .map((brand) => brand.id); // Take first 3 brands
...
async function brandWorkflow() {
  try {
    ...
    const allBrandsPage = await getAllBrands();

    // allBrandsPage returns the items array from getAllBrands()
    if (allBrandsPage.length > 0) {
      console.log("\n2. Getting details for first brand...");
      await getBrandDetail(allBrandsPage[0].id);
...
<Check>
  **Brand ID Stability:** Brand IDs are stable identifiers that you can cache
  and reuse. They won't change unless a brand is completely removed from the
  platform.
</Check>

<Tip>
  You can find brand IDs in the [Channel3
  dashboard](https://trychannel3.com/dashboard/brands) or by using the
  `getBrands()` method to discover available brands programmatically.
</Tip>
...
<Note type="best_practice" title="Error Handling Best Practices">
  Always wrap API calls in `try...catch` blocks in production code. This
  prevents crashes and allows you to provide helpful feedback to users when
...
class ProductSearchApp {
  ...
  async searchProducts(
    ...
    try {
      ...
      if (options.brandIds) filters.brand_ids = options.brandIds;
      if (options.priceRange) {
        filters.price = {
          min_price: options.priceRange.min,
          max_price: options.priceRange.max,
        };
      }
  ...
  private handleError(error: any, operation: string): void {
    ...
    if (error instanceof Channel3AuthenticationError) {
      ...
      console.error(
        "Suggestion: Verify your API key in the Channel3 dashboard"
      );
    } else if (error instanceof Channel3ValidationError) {
      ...
      console.error(
        "Suggestion: The product may have been removed or doesn't exist"
      );
    } else if (error instanceof Channel3ServerError) {
  ...
  async demonstrateBrandOperations(): Promise<void> {
    try {
      ...
      const brandsPage = await this.client.getBrands("athletic", 1, 10);
      console.log(
        `   ✅ Found ${brandsPage.total} athletic brands (showing ${brandsPage.items.length})`
      );

      brandsPage.items.slice(0, 3).forEach((brand) => {
        console.log(`   - ${brand.name} (ID: ${brand.id})`);
      });

      // Get detailed brand information
      if (brandsPage.items.length > 0) {
        const brandId = brandsPage.items[0].id;
        console.log(
          `   Getting details for brand: ${brandsPage.items[0].name}...`
        );
        ...
        try {
          ...
          if (detailedBrand.description) {
            console.log(
              `      Description: ${detailedBrand.description.substring(
                0,
                100
              )}...`
            );
          ...
          console.log(
            `      Product Count: ${detailedBrand.product_count || "N/A"}`
          );
        } catch (error) {
        ...
        const brandIds = brandsPage.items.slice(0, 2).map((b) => b.id);
        console.log(
          `   Searching products from brands: ${brandIds.join(", ")}`
        );
        ...
        console.log(
          `   Found ${brandProducts.length} products from selected brands:`
        );
  ...
  async runDemo(): Promise<void> {
    ...
    <Tip>
      You can find brand IDs in the [Channel3
      dashboard](https://trychannel3.com/dashboard/brands).
    </Tip>;
    ...
    if (sneakers.length > 0 && sneakers[0].id) {
      ...
      if (productDetail) {
        ...
        console.log(
          `   Description: ${productDetail.description.substring(0, 200)}...`
        );
        console.log(
          `   Materials: ${productDetail.materials?.join(", ") || "N/A"}`
        );
        console.log(`   Features: ${productDetail.key_features.join(", ")}`);
        console.log(
          `   Categories: ${productDetail.categories?.join(", ") || "N/A"}`
        );
        ...
        if (productDetail.specifications) {
          ...
          Object.entries(productDetail.specifications).forEach(
            ([key, value]) => {
              console.log(`     ${key}: ${value}`);
            }
          );
...
- **Filters**: Brand IDs, nested price range, gender, availability filtering
...
<Check>
  **Production Ready:** This example includes proper error handling, logging,
  and structured code that you can adapt for production applications.
</Check>
