...
filters = SearchFilters(brand_ids=[brand_id_1, brand_id_2])
products = client.search(query="running shoes", filters=filters)
```

<Tip>
  Use brand IDs from the brands endpoints or dashboard. Names are not accepted.
</Tip>

### Price

```python
from channel3_sdk import SearchFilterPrice

filters = SearchFilters(
    price=SearchFilterPrice(
        min_price=25.0,
        max_price=150.0
    )
)
...
```python
filters = SearchFilters(gender="female")
...
<Tip>
  The availability filter should be an array containing one or more of the
  following values: - "InStock" - "LimitedAvailability" - "PreOrder" -
  "BackOrder" - "SoldOut" - "OutOfStock" - "Discontinued" - "Unknown" Example:
  `availability=["InStock", "PreOrder"]` to find products that are either in
  stock or available for pre-order.
</Tip>
...
filters = SearchFilters(
    ...
    price=SearchFilterPrice(min_price=50.0, max_price=200.0),
    gender="male",
...
brands_page = client.get_brands()
print(f"Total brands: {brands_page.total} (page {brands_page.page} size {brands_page.size})")

for brand in brands_page.items:
...
brands_page = client.get_brands(query="nike")

# Get brands with pagination
brands_page = client.get_brands(page=2, size=50)
...
brands_page = client.get_brands(query="athletic")
brand_ids = [brand.id for brand in brands_page.items[:3]]  # Take first 3 brands

# Use brand IDs in search filters
filters = SearchFilters(brand_ids=brand_ids)
products = client.search(query="running shoes", filters=filters)
...
<Tip>
  Brand IDs are stable identifiers that you can cache and reuse. They won't
  change.
</Tip>
...
async def main():
    try:
        ...
        filters = SearchFilters(
            brand_ids=["brand_id_1", "brand_id_2"],
            price=SearchFilterPrice(min_price=80.0, max_price=200.0),
            gender="male",
            availability=["InStock"]
        )
        ...
        for i, product in enumerate(products, 1):
            ...
            if product.price:
                print(f"   Price: ${product.price.price} {product.price.currency}")
        ...
        brands_page = await client.get_brands(query="athletic", page=1, size=10)
        print(f"Found {brands_page.total} athletic brands (showing {len(brands_page.items)} items)")

        for brand in brands_page.items[:3]:
            print(f"- {brand.name} (ID: {brand.id})")

        # Get detailed brand information
        if brands_page.items:
            brand_id = brands_page.items[0].id
            try:
        ...
        if brands_page.items:
            brand_ids = [brand.id for brand in brands_page.items[:2]]
